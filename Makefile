...
run:
	go run -ldflags "-s -w" cmd/podinfo/* \
...
build:
	GIT_COMMIT=$$(git rev-list -1 HEAD) && CGO_ENABLED=0 go build  -ldflags "-s -w" -a -o ./bin/podinfo ./cmd/podinfo/*
	GIT_COMMIT=$$(git rev-list -1 HEAD) && CGO_ENABLED=0 go build  -ldflags "-s -w" -a -o ./bin/podcli ./cmd/podcli/*
